...
def get_nan_num(spans1,spans2,spans3,spans4,spans5):
    nan_num =0
    if len(spans1)==0:
        nan_num += 1
...
def get_spans_by_vote(spans1,spans2,spans3,spans4,spans5,k=3):
    '''
...
def multi_vote(file1,file2,file3,file4,file5,file_out,k=3):
    data_list1 =None
    data_list2 = None
    data_list3 = None
    data_list4 = None
    ...
    if data_list1 is not None and data_list5 is not None:
        if len(data_list1) != len(data_list5):
            print("数据个数不一致")
    new_data_list = []
    for data1, data2,data3,data4,data5 in zip(data_list1,data_list2,data_list3,data_list4,data_list5):
        spans1 = data1["pred_spans"]
        spans2 = data2["pred_spans"]
        spans3 = data3["pred_spans"]
        spans4 = data4["pred_spans"]
        spans5 = data5["pred_spans"]
        nan_num = get_nan_num(spans1,spans2,spans3,spans4,spans5)
        if nan_num>=k:
            data = {
            ...
            统计spans 里面每个类别出现
            '''
            vote_spans = get_spans_by_vote(spans1,spans2,spans3,spans4,spans5,k)
...
if __name__ == '__main__':
    ...
    multi_vote(file1, file2, file3, file4, file5, file_out, k=3)
