...
def get_nan_num(spans1,spans2,spans3):
    nan_num =0
    if len(spans1)==0:
        nan_num += 1
...
def get_spans_by_vote(spans1,spans2,spans3,k=2):
    '''
    ...
    all_drange_set = set(drange_label_map1.keys()) | set(drange_label_map2.keys()) | set(drange_label_map3.keys())
...
def multi_vote(file1,file2,file3,file_out,k=2):
    data_list1 =None
    data_list2 = None
    data_list3 = None
    ...
    new_data_list = []
    for data1, data2, data3 in zip(data_list1, data_list2, data_list3):
        spans1 = data1["pred_spans"]
        spans2 = data2["pred_spans"]
        spans3 = data3["pred_spans"]

        nan_num = get_nan_num(spans1, spans2, spans3)
        if nan_num>=k:
            data = {
            ...
            统计spans 里面每个类别出现
            '''
            vote_spans = get_spans_by_vote(spans1,spans2,spans3,k)
...
if __name__ == '__main__':
    ...
    multi_vote(file1, file2, file3, file_out, k=2)
