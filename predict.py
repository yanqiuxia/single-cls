...
class Predict(object):
    ...
    def get_span_dranges_masks(self, spans):
        ...
        for span in spans:
            ...
            if end_offset<511:
                ...
                print("span start offset 超过512范围！请检查！")
                drange = [0, 0]
                span_dranges.append(drange)
                span_masks.append(1)
    ...
    def _model_predict(self, text_list,spans_list):
        ...
        loss, spans_pred_ids,spans_prods = self.model(
        ...
        '''
        result_spans_list = self._generate_label(spans_list, spans_pred_ids)

        return result_spans_list

    def _generate_label(self, spans_list, spans_pred_ids):
        '''
        ...
        for spans, span_pred_ids in zip(spans_list,spans_pred_ids ):
